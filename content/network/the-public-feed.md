...
| `AntibodyCorroborated` | yes | corroboration-count change |
| `AntibodyMatured` | yes | status change to ACTIVE |
| `AntibodyMatched` | yes | match event with value-at-risk |
| `AntibodyChallenged` | yes | status change in feed |
| `AntibodySlashed` | yes | status change in feed |
| `AntibodyExpired` | yes | status change in feed |
| `CheckSettled` (per-check telemetry) | yes (aggregated) | feeds the dashboard, not the antibody feed |
| reputation updates | yes | publisher dashboard updates |
...
- **Cryptographic equivalence with the Registry.** The feed is a derivative; read the Registry directly for trust-critical decisions.
...
1. Take the `keccakId` (or matcher hash) from the feed item.
2. Read the antibody from the Registry on your own RPC and decode it with `decodeAntibody`.
...
The indexer is a Zephyrus PHP app reading from Base via JSON-RPC, writing to Postgres, exposing the same RSS / JSON Feed / webhook interface. Deploy it anywhere that can run PHP 8.4 + Postgres.
...
- **[Network: Registry on Base](/network/registry-on-base/)**, the canonical source of truth.
